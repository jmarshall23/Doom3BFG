...
void idTypeInfoGen::WriteTypeInfo( const char *fileName ) const {
	...
	for ( i = 0; i < classes.Num(); i++ ) {
		...
		{
			file->WriteFloatString("intptr_t %s::Invoke(%s *cls, const char *functionName) {\n", typeInfoName.c_str(), typeName.c_str());
			//file->WriteFloatString("\tTypeInfoVariableArgs();\n");
			file->WriteFloatString("\tint functionNameHash = idStr::Hash(functionName);\n");
			for (j = 0; j < info->functions.Num(); j++) {
				...
				file->WriteFloatString("\tif(functionNameHash == %d) { // %s\n", hash, varName);

				if (!strstr(info->functions[j].returnType.c_str(), "*") && info->functions[j].returnType != "int " && info->functions[j].returnType != "short ") {
					file->WriteFloatString("\t\tcls->%s();\n", varName);
					file->WriteFloatString("\t\treturn 0;\n", varName);
				}
				else {
					file->WriteFloatString("\t\treturn (intptr_t)cls->%s();\n", varName);
				}
				file->WriteFloatString("\t};\n");
			}
			file->WriteFloatString("\treturn 0;\n\n");
